...
const MapDepartamentos = ({ data, isScale = false, departamentos = [] }) => {
  ...
  // Calculate total values for the country
          ...
          <div className="h-full flex flex-col">
            <div className="mb-6">
              <p className="text-gray-700 text-sm leading-relaxed">
                Selecciona un departamento para ver sus cifras de biodiversidad y explorar su información detallada.
              </p>
            </div>
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className="text-sm font-semibold text-gray-800">
                      {selectedDepartment.properties.n_especies?.toLocaleString() || 0}
                    </span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${((selectedDepartment.properties.n_especies || 0) / Math.max(...data.features.map(f => f.properties.n_especies || 0))) * 100}%`
                      }}
                    ></div>
                  </div>
                  <div className="text-xs text-gray-500 mt-1">
                    {(((selectedDepartment.properties.n_especies || 0) / totalSpecies) * 100).toFixed(1)}% del total
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className="text-sm font-semibold text-gray-800">
                      {selectedDepartment.properties.n_registros?.toLocaleString() || 0}
                    </span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${((selectedDepartment.properties.n_registros || 0) / Math.max(...data.features.map(f => f.properties.n_registros || 0))) * 100}%`
                      }}
                    ></div>
                  </div>
                  <div className="text-xs text-gray-500 mt-1">
                    {(((selectedDepartment.properties.n_registros || 0) / totalRecords) * 100).toFixed(1)}% del total
                  </div>
                </div>
              </div>
            </div>
...
export default MapDepartamentos
